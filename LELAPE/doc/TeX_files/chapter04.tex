...
\begin{itemize}
	...
	\end{itemize}
	\textbf{NOTE:} Alternatively, \textbf{NSBU} and \textbf{NMU2} can be replaced by two Vector{Int} variables if there were several reading rounds. Thus, the \(l\)-th elements of the vector indicates the number of single bit upsets and 2-bit multiple cell upsets observed during the \(k\)-th round. Both vectors must be of the same size and no negative values are allowed.
