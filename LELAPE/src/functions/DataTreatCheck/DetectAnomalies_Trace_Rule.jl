...
# for further details.
#
function DetectAnomalies_Trace_Rule(   
                            TraceRuleLength::Int, 
...
function DetectAnomalies_Trace_Rule(   
    ...
    elseif TraceRuleLength == 2
        ...
        LowTraceSet = sort(union([UInt64(2^n+2^m) for n=0:NBits-1, m=0:NBits-1 if m!=n]))

    elseif TraceRuleLength == 3

        LowTraceSet = sort(union([UInt64(2^n+2^m+2^p) for n=0:NBits-1, m=0:NBits-1, p=0:NBits-1 if (m!=n) & (m!=p) & (n!=p)]))
    ...
    Matrix4SMCURULE = convert.(UInt64, reshape([PossibleCandidates; Histogram[PossibleCandidates.+1]], :, 2))
