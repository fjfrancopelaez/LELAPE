...
function POS_MCU_Indexes(   ADDRESS::Vector{UInt64}, 
                            CYCLES::Vector{UInt64}, 
                            POSDVMarkers::Vector{UInt64}, 
                            LimitMCUSize::Int=100)::Array{Int, 2}
    # This function looks for pairs of ADDRESSES that subtracted yield one of the
    # identified POSDVMarkers. In the case of success, a matrix is returned. The
    # matrix has as many rows as MCU candidates and as many columns as the size
    ...
    GroupsOfEventsDraft = zeros(UInt64, div(length(ADDRESS),2),LimitMCUSize)
...
function POS_MCU_Indexes(ADDRESS::Vector{UInt64}, POSDVMarkers::Vector{UInt64}, LimitMCUSize::Int=1000)::Array{Int64, 2}

    return POS_MCU_Indexes(ADDRESS, ones(UInt64, length(ADDRESS)), POSDVMarkers, LimitMCUSize)
