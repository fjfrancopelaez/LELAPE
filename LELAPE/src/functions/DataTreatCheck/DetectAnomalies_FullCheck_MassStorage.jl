...
function DetectAnomalies_FullCheck_MassStorage(   DATA::Array{UInt64, 2},
    WordWidth::Int,
    LN0::Int,
    Operation::String,
    TraceRuleLength::Int, 
...
#       and is only used to stop the program if unreallistic events occur. Initially set to 200.
#
# The funcion returns several Nx2 UInt64 matrix. The first column contains the anomalously repeated values of the 
# DV SET compatible with the SelfConsistency test. The second one contains the number of times they appear
...
# Removed from old version
# Matrix4SelfConsistency = convert.(UInt64, reshape([PropoCandidatesFromSC; Histogram[PropoCandidatesFromSC.+1]], :, 2))
#
    Matrix4SelfConsistency = convert.(UInt64, reshape([PropoCandidatesFromSC; Histogram[[findfirst(Histogram[:,1].==x) for x in PropoCandidatesFromSC], 2]] , :, 2))
# Now, the MCU Rule:
    ...
    if length(PrevCandidates) > 1
        ...
        length(MCU_Index) != 0 && length(MCU_Index[1, :]) > 2 ?
        LargeMCUIndexes = findall(MCU_Index[:,3].!=0) :
        LargeMCUIndexes = UInt64[]
# These are the rows where there are more than 2 SBUs.

        PossibleCandidates = UInt64[]
    ...
    #ConfirmPossibleCandidates = union(PossibleCandidates)
        ...
        Matrix4SMCURULE = convert.(UInt64, reshape(ConfirmedCandidates', :, 2))
...
#Matrix4SMCURULE = convert.(UInt64, reshape([PossibleCandidates; [Histo]], :, 2))
    ...
    if length(PossibleCandidates)>0
        ...
        Matrix4SHUFFLERULE = zeros(UInt64, 0, 2)
...
#The lowest number of bits able to represent all the addresses.    
    ...
    if TraceRuleLength == 1
        ...
        LowTraceSet = sort(union([UInt64(2^n) for n=0:NBits-1]))

    elseif TraceRuleLength == 2

        LowTraceSet = sort(append!(
                    union([UInt64(2^n) for n=0:NBits-1]), 
                    union([UInt64(2^n+2^m) for n=0:NBits-1, m=0:NBits-1 if m!=n]) 
                        )
                        )
    ...
    elseif TraceRuleLength == 3
        ...
        LowTraceSet = sort(append!(
                    union([UInt64(2^n) for n=0:NBits-1]), 
                    union([UInt64(2^n+2^m) for n=0:NBits-1, m=0:NBits-1 if m!=n]),
                    union([UInt64(2^n+2^m+2^p) for n=0:NBits-1, m=0:NBits-1, p=0:NBits-1 if (m!=n) & (m!=p) & (n!=p)]) 
            )   )
    else

        println("Nonsense value for trace.")
        return zeros(UInt64,  0, 2)
...
# 2023-02-07: Bug fixed. See https://github.com/fjfrancopelaez/LELAPE/issues/2
    ...
    Matrix4TRACERULE = convert.(UInt64, reshape([PossibleCandidates; Histogram[PossibleCandidates.+1]], :, 2))
